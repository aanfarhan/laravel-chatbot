...
### Channel settings
The resolved configuration for a [[channel]], read through one module. Settings fall into two families. **Tiered settings** (model, throttle limits, [[freshness-window]], prompt, greeting) follow the rule **per-channel value → global value → default**. **Channel-only settings** (summary, extractor allowlist, extractor timeout and size cap) have no global tier or shared default — a channel either sets them or it does not — so they resolve from the per-channel key alone, yielding `null`/empty when absent. Both families share the same module and the same strictness: a config value of the wrong type fails loud (the framework's typed getters throw) rather than silently degrading; only a *missing* key falls through (to the next tier for tiered settings, to the empty result for channel-only ones). Tiered defaults live once in a flat constants holder, referenced by both the published config file and the resolver. The module hides only the merge rule — pure global scalars with no per-channel dimension (e.g. token cap, stream duration, tool caps) are read directly through the framework's typed config getters, not fronted here, to avoid a shallow pass-through interface. Distinct from the in-memory overrides set by the fluent builder on the `Chatbot` facade, which take precedence over this resolver but are not owned by it.
...
### Context envelope
The signed, short-lived token minted at page render that carries the channel's context payload plus the binding metadata the endpoint re-verifies (user, route, channel, expiry, version) and the channel's signed-in capability set: tool allowlist, client-extractor allowlist, and per-channel extractor overrides.
